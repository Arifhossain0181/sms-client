  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setStudentId(me.id);
        setExams(data.filter((e) => e.resultPublished));
      } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Download your published report cards.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {exams.map((exam) => (
          <div key={exam.examId} className="rounded-2xl border border-border/60 bg-card/80 p-6 shadow-soft space-y-3">
            ...
            <p className="text-xs text-muted-foreground">Status: Published</p>
            {studentId && (
              <a
                href={`/api/v1/exams/${exam.examId}/students/${studentId}/report-card`}
                target="_blank"
