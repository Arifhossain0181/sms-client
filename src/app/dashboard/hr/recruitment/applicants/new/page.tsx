...
export default function NewApplicantPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/hr/recruitment/jobs/${jobId}`);
    } catch (err: any) {
      alert(err?.response?.data?.message ?? "Failed to add applicant");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-2xl rounded-3xl border border-white/30 dark:border-white/10 shadow-2xl shadow-slate-200/40 dark:shadow-none overflow-hidden">
          <div className="relative px-6 sm:px-8 py-6 bg-gradient-to-r from-sky-50 via-indigo-50 to-violet-50 dark:from-sky-500/10 dark:via-indigo-500/10 dark:to-violet-500/10 border-b border-white/40 dark:border-white/5 overflow-hidden">
            ...
            <div className="relative flex flex-wrap items-center gap-4">
              {jobId ? (
                <Link
                  href={`/dashboard/hr/recruitment/jobs/${jobId}`}
                ...
                <Link
                  href="/dashboard/hr/recruitment/applicants"
                  className="flex items-center gap-1 text-sm text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors"
                >
                  <ArrowLeft className="h-4 w-4" /> Back
                </Link>
              ...
              <div className="flex-1">
                ...
                <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
                  {fetchingJob
                    ? "Loading job details..."
                    : job
                      ? `Adding applicant for: ${job.title} · ${job.designation}`
                      : "Add a new applicant to a job posting"}
                </p>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                <h3 className="font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                  <Plus className="w-4 h-4 text-indigo-400" /> Applicant Details
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">Resume URL</label>
                    <input
                ...
                <div>
                  <label className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">Cover Letter</label>
                  <textarea
                    value={form.coverLetter}
                    onChange={(e) => updateForm("coverLetter", e.target.value)}
                    rows={4}
                    placeholder="Brief cover letter or notes about the applicant..."
                ...
                <div>
                  <label className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">Internal Notes</label>
                  <textarea
                    value={form.notes}
                    onChange={(e) => updateForm("notes", e.target.value)}
                    rows={3}
                    placeholder="Optional internal notes for HR review..."
                    className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white/60 dark:bg-white/5 px-3 py-2 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-400/50 resize-none"
                  />
                </div>
              </motion.div>
              ...
              >
                ...
                {jobId ? (
                  <Link href={`/dashboard/hr/recruitment/jobs/${jobId}`}>
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      type="button"
                  ...
                  <Link href="/dashboard/hr/recruitment/applicants">
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      type="button"
