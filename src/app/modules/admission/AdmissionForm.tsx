...
const schema = z
  .object({
    // StudentForm fields
    studentFormName: z.string().optional(),
    studentFormEmail: z.string().email("Enter a valid student email").optional(),
    studentFormPassword: z.string().optional(),
    studentFormPhone: z.string().optional(),
    studentFormClassId: z.string().optional(),
    studentFormRollNumber: z.string().optional(),
    studentFormGender: z.enum(["MALE", "FEMALE", "OTHER"]).optional(),
    studentFormAddress: z.string().optional(),
    studentFormDateOfBirth: z.string().optional(),
    studentFormBloodGroup: z.string().optional(),
    studentFormGuardianName: z.string().optional(),
    studentFormGuardianPhone: z.string().optional(),
    studentFormGuardianEmail: z.string().optional(),
    studentFormGuardianRelation: z.string().optional(),
    // Admission fields
...
// Helper component for animated form fields
function Field({ delay, span = 1, children }: { delay?: number; span?: number; children: React.ReactNode }) {
  return (
    <motion.div
...
type DocumentType = "photo" | "birthCert" | "guardianNid" | "fatherNid" | "fatherPhoto" | "motherNid" | "motherPhoto";

function DocumentUpload({ label, url, type, accept, loading, onUpload }: { label: string; url: string | null; type: DocumentType; accept: string; loading: boolean; onUpload: (file: File, type: DocumentType) => void }) {
  return (
    <Field span={2}>
      ...
      <label className="mt-2 flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-slate-200 dark:border-white/10 px-4 py-3 cursor-pointer hover:border-indigo-400 transition">
        <span className="text-sm text-slate-500 truncate">{url ? "✓ Uploaded" : "Upload image"}</span>
        <span className="text-xs font-semibold text-indigo-600">Browse</span>
        <input type="file" accept={accept} className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) onUpload(file, type); }} />
      </label>
      {loading && <p className="text-xs text-slate-400 mt-1"><Loader2 className="inline h-3 w-3 animate-spin" /> Uploading...</p>}
    </Field>
  );
}
...
export default function Admission() {
  ...
  } = useForm<FormInput>({ resolver: zodResolver(schema) as any, defaultValues: { payNow: true } });
  ...
  const saveDraft = () => {
    ...
    const values = {
      ...
      // StudentForm added
      studentFormName: watch("studentFormName"),
      studentFormEmail: watch("studentFormEmail"),
      studentFormPassword: watch("studentFormPassword"),
      studentFormPhone: watch("studentFormPhone"),
      studentFormClassId: watch("studentFormClassId"),
      studentFormRollNumber: watch("studentFormRollNumber"),
      studentFormGender: watch("studentFormGender"),
      studentFormAddress: watch("studentFormAddress"),
      studentFormDateOfBirth: watch("studentFormDateOfBirth"),
      studentFormBloodGroup: watch("studentFormBloodGroup"),
      studentFormGuardianName: watch("studentFormGuardianName"),
      studentFormGuardianPhone: watch("studentFormGuardianPhone"),
      studentFormGuardianEmail: watch("studentFormGuardianEmail"),
      studentFormGuardianRelation: watch("studentFormGuardianRelation"),
      // Upload
      photoUrl,
      birthCertUrl,
      guardianNidUrl,
  ...
  const restoreDraft = () => {
    ...
    try {
      ...
      reset({
        ...
        studentFormName: draft.studentFormName,
        studentFormEmail: draft.studentFormEmail,
        studentFormPassword: draft.studentFormPassword,
        studentFormPhone: draft.studentFormPhone,
        studentFormClassId: draft.studentFormClassId,
        studentFormRollNumber: draft.studentFormRollNumber,
        studentFormGender: draft.studentFormGender,
        studentFormAddress: draft.studentFormAddress,
        studentFormDateOfBirth: draft.studentFormDateOfBirth,
        studentFormBloodGroup: draft.studentFormBloodGroup,
        studentFormGuardianName: draft.studentFormGuardianName,
        studentFormGuardianPhone: draft.studentFormGuardianPhone,
        studentFormGuardianEmail: draft.studentFormGuardianEmail,
        studentFormGuardianRelation: draft.studentFormGuardianRelation,
      });
  ...
  useEffect(() => {
    const checkExistingApplication = async () => {
      try {
        ...
        if (applications.length > 0) setExistingApplication(applications[applications.length - 1]);
      } catch {
  ...
  const submitDraftForStripe = async (sessionId: string, amountTotal?: number | null) => {
    ...
    await api.post("/admission/apply", {
      ...
      paymentAmount: amountTotal ?? (draft as any).paymentAmount,
      transactionId: sessionId,
      photoUrl: (draft as any).photoUrl ?? photoUrl ?? undefined,
      birthCertUrl: (draft as any).birthCertUrl ?? birthCertUrl ?? undefined,
      guardianRelation: (draft as any).guardianRelation,
      guardianNidUrl: (draft as any).guardianNidUrl ?? guardianNidUrl ?? undefined,
      fatherNid: (draft as any).fatherNidUrl ?? fatherNidUrl ?? undefined,
      fatherPhotoUrl: (draft as any).fatherPhotoUrl ?? fatherPhotoUrl ?? undefined,
      motherNid: (draft as any).motherNidUrl ?? motherNidUrl ?? undefined,
      motherPhotoUrl: (draft as any).motherPhotoUrl ?? motherPhotoUrl ?? undefined,
    });
  ...
  const onSubmit: SubmitHandler<FormInput> = async (data) => {
    ...
    try {
      ...
      await api.post("/admission/apply", {
        ...
        guardianNidUrl,
        fatherNid: fatherNidUrl,
        fatherPhotoUrl,
        motherNid: motherNidUrl,
        motherPhotoUrl,
      });
      ...
      toast.error((err as any)?.response?.data?.message || "Application failed");
    }
  };
      ...
      <div className="relative mx-auto max-w-4xl">
        ...
        >
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400 max-w-xl mx-auto">
            Complete both the Student Form and Admission Form
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="p-6 sm:p-8 lg:p-10 space-y-10">
            {/* ===== NEW: StudentForm info section (ADDED) ===== */}
            <div>
              ...
              <div className="mt-5 grid gap-5 md:grid-cols-2">
                <Field delay={0.05}>
                  ...
                  <input {...register("studentFormName")} className={inputCls} placeholder="Full name" />
                  {errors.studentFormName && <p className={errCls}>{errors.studentFormName.message}</p>}
                </Field>
                ...
                <Field delay={0.1}>
                  ...
                  <input
                    type="email"
                    {...register("studentFormEmail")}
                    className={inputCls}
                    placeholder="student@email.com"
                  />
                  {errors.studentFormEmail && <p className={errCls}>{errors.studentFormEmail.message}</p>}
                </Field>
                ...
                <Field delay={0.2}>
                  <label className={labelCls}>
                    <Lock className="h-3.5 w-3.5 text-indigo-500" /> Password (optional)
                  </label>
                  <input type="password" {...register("studentFormPassword")} className={inputCls} />
                </Field>
                ...
                <Field delay={0.2}>
                  ...
                  <input {...register("studentFormPhone")} className={inputCls} placeholder="01XXXXXXXXX" />
                  {errors.studentFormPhone && <p className={errCls}>{errors.studentFormPhone.message}</p>}
                </Field>
                ...
                <Field delay={0.25}>
                  ...
                  <select {...register("studentFormClassId")} className={inputCls} disabled={loadingClasses}>
                  ...
                  {errors.studentFormClassId && <p className={errCls}>{errors.studentFormClassId.message}</p>}
                </Field>
                ...
                <Field delay={0.3}>
                  ...
                  <input {...register("studentFormRollNumber")} className={inputCls} placeholder="e.g. 12" />
                  {errors.studentFormRollNumber && (
                    <p className={errCls}>{errors.studentFormRollNumber.message}</p>
                  )}
                </Field>
                ...
                <Field delay={0.35}>
                  ...
                  <select {...register("studentFormGender")} className={inputCls}>
                    <option value="">Select</option>
                    <option value="MALE">Male</option>
                    <option value="FEMALE">Female</option>
                    <option value="OTHER">Other</option>
                  </select>
                  {errors.studentFormGender && <p className={errCls}>{errors.studentFormGender.message}</p>}
                </Field>
                ...
                <Field delay={0.4}>
                  ...
                  <input type="date" {...register("studentFormDateOfBirth")} className={inputCls} />
                  {errors.studentFormDateOfBirth && (
                    <p className={errCls}>{errors.studentFormDateOfBirth.message}</p>
                  )}
                </Field>
                ...
                <Field delay={0.45}>
                  ...
                  <select {...register("studentFormBloodGroup")} className={inputCls}>
                ...
                <Field delay={0.5} span={2}>
                  ...
                  <textarea
                    {...register("studentFormAddress")}
                    rows={3}
                    className={inputCls}
                    placeholder="Full address"
                  />
                  {errors.studentFormAddress && <p className={errCls}>{errors.studentFormAddress.message}</p>}
                </Field>
              </div>
            </div>

            {/* ===== Existing Admission info section (kept) ===== */}
            <div>
              <h2 className={sectionTitleCls}>
              ...
              <div className="mt-5 grid gap-5 md:grid-cols-2">
                <Field delay={0.05}>
                  <label className={labelCls}>
                    <User className="h-3.5 w-3.5 text-indigo-500" /> Student Name (Admission)
                  </label>
                ...
                <Field delay={0.1}>
                  <label className={labelCls}>
                    <Mail className="h-3.5 w-3.5 text-indigo-500" /> Student Email (for login)
                  </label>
                ...
                <Field delay={0.15}>
                  <label className={labelCls}>
                    <Calendar className="h-3.5 w-3.5 text-indigo-500" /> Birth Date
                  </label>
                ...
                <Field delay={0.3}>
                  <label className={labelCls}>
                    <BookOpen className="h-3.5 w-3.5 text-violet-500" /> Religion (optional)
                  </label>
                ...
                <Field delay={0.35} span={2}>
                  <label className={labelCls}>
                    <MapPin className="h-3.5 w-3.5 text-indigo-500" /> Address (Admission)
                  </label>
                ...
                <Field delay={0.45} span={2}>
                  ...
                  <label className="mt-2 group flex items-center justify-between gap-3 rounded-xl border-2 border-dashed border-slate-200 dark:border-white/10 bg-white/50 dark:bg-white/5 backdrop-blur px-4 py-3 cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/40 dark:hover:bg-indigo-500/5 transition">
                    ...
                    <span className="shrink-0 text-xs font-semibold text-indigo-600 dark:text-indigo-300">Browse</span>
                    <input
                      type="file"
                      accept="image/*"
                      className="hidden"
                ...
                <Field delay={0.5} span={2}>
                  <label className={labelCls}>
                    <FileText className="h-3.5 w-3.5 text-emerald-500" /> Birth Certificate <span className="text-slate-400 font-normal">(optional)</span>
                  </label>
            ...
            {/* ===== Guardian (kept as Admission keys) ===== */}
            <div className="border-t border-slate-200 dark:border-white/10 pt-8">
              ...
              <div className="mt-5 grid gap-5 md:grid-cols-2">
                ...
                <DocumentUpload label="Guardian NID image" url={guardianNidUrl} type="guardianNid" accept="image/*,.pdf" loading={uploading.guardianNid} onUpload={uploadDocument} />
                <DocumentUpload label="Father NID image" url={fatherNidUrl} type="fatherNid" accept="image/*,.pdf" loading={uploading.fatherNid} onUpload={uploadDocument} />
                <DocumentUpload label="Father Photo" url={fatherPhotoUrl} type="fatherPhoto" accept="image/*" loading={uploading.fatherPhoto} onUpload={uploadDocument} />
                <DocumentUpload label="Mother NID image" url={motherNidUrl} type="motherNid" accept="image/*,.pdf" loading={uploading.motherNid} onUpload={uploadDocument} />
                <DocumentUpload label="Mother Photo" url={motherPhotoUrl} type="motherPhoto" accept="image/*" loading={uploading.motherPhoto} onUpload={uploadDocument} />
              </div>
            </div>
                  ...
                  >
                    <div className="mt-5 grid gap-5 md:grid-cols-2">
                      <Field delay={0.05}>
                        ...
                        {errors.paymentMethod && <p className={errCls}>{errors.paymentMethod.message as any}</p>}
                      </Field>

                      <Field delay={0.1}>
                        <label className={labelCls}>Amount</label>
                        <input type="number" {...register("paymentAmount")} className={inputCls} placeholder="0.00" />
                        {errors.paymentAmount && <p className={errCls}>{errors.paymentAmount.message as any}</p>}
                      </Field>
